...
namespace TSWMRepository
{
    ...
    public class BSDataHelper
    {
        ...
        /// </summary>
        public const int REQ_POS_DATA_COUNT_H = 4;
        /// <summary>
        /// Кол-во данных в теле запроса (2 of 2 bytes) 
        /// </summary>
        public const int REQ_POS_DATA_COUNT_L = 5;
        /// <summary>
        ...
        public byte[] DataRequest(byte sensorType)
        {
            ...
            int requestSize = header.Length + header[REQ_POS_DATA_COUNT_L] + 1; // +1 for checksum
            byte[] request = new byte[requestSize];
            header.CopyTo(request, 0);
            request[REQ_POS_OP1] = sensorType;
            request[requestSize - 1] = CRC.ComputeXORChechsum(new byte[] { request[REQ_POS_OP1] }); //CRC Body hash 
        ...
        public byte[] SetParams(byte sensorType, byte freq, byte coeff, byte threshold)
        {
            ...
            request[requestSize - 1] = CRC.ComputeXORChechsum(new byte[] { request[REQ_POS_OP1], request[REQ_POS_OP2], request[REQ_POS_OP3], request[REQ_POS_OP4] }); //CRC Body hash 
            return request;
        }
        ...
        private byte[] prepareHeader(BSCommand cmd)
        {
            ...
            header[REQ_POS_DATA_COUNT_H] = 0x00; //DATA_COUNT High byte (2 Byte)
            header[REQ_POS_DATA_COUNT_L] = dataCount; //DATA_COUNT Low byte (2 Byte)
            header[REQ_POS_HEADER_CRC] = CRC.ComputeXORChechsum(new byte[]{ header[REQ_POS_DEV_ADDR], header[REQ_POS_PACKAGE_NUM],header[REQ_POS_CMD],header[REQ_POS_DATA_COUNT_H],header[REQ_POS_DATA_COUNT_L] }); //CRC header hash
    ...
    {
        /* XOR  START */
        public static byte ComputeXORChechsum(params byte[] data)
        {
            byte key = 0;
